<?php
  ...
  if (isset($_POST['new_enable']) || isset($_POST['new_disable'])) {
    ...
    try {
      ...
      if (!empty($_POST['products'])) {
        foreach ($_POST['products'] as $product_id) {
          ...
          $product->data['new'] = !empty($_POST['new_enable']) ? 1 : 0;
          $product->save();
        }
      }
...
<?php
  ...
  if (!empty($_GET['query'])) {
    ...
    $products_query = database::query(
      "select p.id, p.status, p.new, p.sold_out_status_id, p.image, p.quantity, p.date_valid_from, p.date_valid_to, pi.name,
      (
    ...
    function admin_catalog_category_products($category_id=0, $depth=1) {
      ...
      while ($product=database::fetch($products_query)) {
        ...
        $output .= '<tr class="'. (!$product['status'] ? ' semi-transparent' : null) .'">' . PHP_EOL
                 ...
                 . '  <td>'. functions::draw_fonticon('fa-circle', 'style="color: '. (!empty($product['status']) ? '#88cc44' : '#ff6644') .';"') .'
      ...
      <p>
        <ul class="list-inline">
          ...
          <li>
            ...
            <div class="btn-group">
              <?php echo functions::form_draw_button('new_enable', language::translate('title_new_enable', 'Enable'), 'submit', '', 'on'); ?>
              <?php echo functions::form_draw_button('new_disable', language::translate('title_new_disable', 'Disable'), 'submit', '', 'off'); ?>
            </div>
          </li>
          <li>
            <div>
