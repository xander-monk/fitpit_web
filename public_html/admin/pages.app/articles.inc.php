...
<ul class="list-inline pull-right">
  <li><?php echo functions::form_draw_link_button(document::link('', array('doc' => 'edit_article'), true), language::translate('title_create_new_article', 'Create New Article'), '', 'add'); ?></li>
</ul>

<h1><?php echo $app_icon; ?> <?php echo language::translate('title_blog', 'Blog'); ?></h1>
...
<?php
  $pages_query = database::query(
    "select p.*, pi.title from ". DB_TABLE_BLOG ." p
    left join ". DB_TABLE_BLOG_INFO ." pi on (p.id = pi.page_id and pi.language_code = '". language::$selected['code'] ."')
    order by p.date, pi.title;"
  );
...
?>
      <tr class="<?php echo empty($page['status']) ? 'semi-transparent' : null; ?>">
        ...
        <td><a href="<?php echo document::href_link('', array('doc' => 'edit_article', 'article_id' => $page['id']), true); ?>"><?php echo $page['title']; ?></a></td>
        <td class="text-right"><a href="<?php echo document::href_link('', array('doc' => 'edit_article', 'article_id' => $page['id']), true); ?>" title="<?php echo language::translate('title_edit', 'Edit'); ?>"><?php echo functions::draw_fonticon('fa-pencil'); ?></a></td>
      </tr>
<?php
