      ...
      // read
        ...
        foreach ($rows as $key => $erow) {
          ...
          database::query("insert into _excel 
            (id,option_id,manufacturer,category,name,variant,price,quantity,status,sale,short_description) 
            values (
        ...
        while ($row = database::fetch($rows)) {
          ...
          <? foreach ($row as $k => $cell) { ?>
            <? switch ($k) {
                case 'mark':
                    ?>
                    <td  id="cell<?=$row['rid'];?><?=$k;?>" title="cell<?=$row['rid'];?><?=$k;?>" class="cell-<?=$k;?>">
                      <? if($cell == "1") { ?>
                  ...
                  break;
                default:
                  ?><td id="cell<?=$row['rid'];?><?=$k;?>" class="cell-<?=$k;?>"><?=$cell;?></td><?
            }?>
